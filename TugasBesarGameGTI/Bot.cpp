...
// =========================================================================
void initBot(){
	// Menempatkan bot hantu di tiap lantai
    for (int i = 0; i < NUM_FLOORS; i++){
        // Bot hantu Lantai 2 di Ujung Kiri (Node 0), jalan ke Kanan
        if (i == 1){	
            ghostBots[i].x = nodes[0].x;
            ghostBots[i].visionX = 1.0f;
            ghostBots[i].movingRight = true;
            ghostBots[i].targetNodeIndex = 1;
        } else{ // Bot hantu Lantai 1 & 3 di Ujung Kanan (Node 2), jalan ke Kiri
...
bool scanForPlayer(int botIdx){
    ...
    // Hanya Chase jika player di area koridor
