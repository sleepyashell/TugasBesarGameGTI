...
static const float HOVER_AMPLITUDE     = 0.12f;  // amplitudo animasi naik-turun
static const float HOVER_SPEED         = 2.0f;   // kecepatan bobbing 
...
void drawItemHUD()
{
    ...
    if (allRequiredItemsCollected()){
        ...
        hudPrint(startX, startY - 16.0f, "RETURN TO THE GATE");
    }
...
void initItems() {
    ...
    for (int r = 0; r < reqCount; r++) {
        ...
        if (assignedSlot == -1) {
            ...
            // cari floor lain yang masih punya slot kosong
            for (int i = 0; i < availableSlots; i++) {
                if (!slotUsed[i]) {
                    assignedSlot = i;
                    break;
